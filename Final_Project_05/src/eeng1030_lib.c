...
void initEncoder()
{
    ...
    pinMode(GPIOB,4,0); // PB4 is the Button press

    RCC->APB1ENR1 |= (1 << 0); // enable Timer 2
    TIM2->ARR = 159;            // 0 - 159 = 160 outputs
    // Rollover for encoder output. CAN I USE FOR THE ECHO FACTOR? YES, BUT the encoder is 160 long
    TIM2->SMCR = 0b010;
