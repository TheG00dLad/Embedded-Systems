...
// Function prototypes (declare all functions used)
...
void process_adc_data(uint32_t start_index, uint32_t length)
{
    // Simple attenuation - Replace with volume control logic if needed
    for (uint32_t i = 0; i < length; ++i)
    {
        dac_buffer[start_index + i] = adc_buffer[start_index + i] / 2;
    }
}
...
int _write(int file, char *data, int len)
{
    ...
    return len; // Return chars written
}

void eputc(char c)
{
    while( (USART2->ISR & USART_ISR_TXE) == 0); // Wait for TXE
    USART2->TDR = c;
}
